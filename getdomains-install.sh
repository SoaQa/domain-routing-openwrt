...
add_tunnel() {
    ...
    if [ "$TUNNEL" == 'wg' ]; then
        ...
        if apk info | grep -q wireguard-tools; then
    ...
    if [ "$TUNNEL" == 'ovpn' ]; then
        if apk info | grep -q openvpn-openssl; then
...
add_dns_resolver() {
    ...
    if [ "$DNS_RESOLVER" == 'DNSCRYPT' ]; then
        if apk info | grep -q dnscrypt-proxy2; then
...
install_awg_packages() {
    ...
    if apk info | grep -q amneziawg-tools; then
    ...
    if apk info | grep -q kmod-amneziawg; then
    ...
    if apk info | grep -q luci-app-amneziawg; then
